...
// 方案一：由 Surface 创建的 Texture 不允许修改其中的像素。如果有修改像素的需求，需要创建 SDL_TEXTUREACCESS_STREAMING （操作复杂）
// 方案二：直接对 Surface 进行修改，因为它的像素格式与Window一样，然后再由 Surface 创建 Texture （简单易操作）
	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				//Copy loaded/formatted surface pixels  formattedSurface->pitch返回一行像素的总字节数（每个像素 4 个字节）
