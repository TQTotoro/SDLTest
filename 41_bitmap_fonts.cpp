...
class LBitmapFont
{
    public:
		...
		//Generates the font from Texture
    ...
    private:
		//The font texture  不必要，如使用也无妨
		//LTexture* mBitmap;

		//The individual characters in the surface
		SDL_Rect mChars[ 256 ];

		// mNewLine：字符的高度    mSpace：空格的宽度
		int mNewLine, mSpace;
};
...
bool LBitmapFont::buildFont( LTexture* bitmap )
{
	...
	else
	{
		...
		// keep track of the top of the tallest glyph in the sprite sheet
		int top = cellH;

		// baseA keeps track of the offset of the bottom of the capital A glyph
		int baseA = cellH;
			...
			for( int cols = 0; cols < 16; ++cols )
			{
				...
				for( int pCol = 0; pCol < cellW; ++pCol )
				{
					...
					for( int pRow = 0; pRow < cellH; ++pRow )
					{
						...
						{
							// Set the x offset 目的是去除多余的背景色部分
				...
				//Find Bottom of A  所有字符的下侧与 ‘A’ 保持一致
				if( currentChar == 'A' )
				{
					//Go through pixel rows
		...
		//Calculate space 定义空格的宽度
		mSpace = cellW / 2;

		//Calculate new line  以所有字符中最大的高度作为高度的定义
		mNewLine = baseA - top;
...
void LBitmapFont::renderText( int x, int y, std::string text )
{
	...
	if (gBitmapFont.buildFont(&gBitmapTexture))
    //if ( mBitmap != NULL )
    {
		//Temp offsets
		int curX = x, curY = y;
        ...
        for( int i = 0; i < text.length(); ++i )
        {
            ...
            else
            {
                ...
                //Show the character
				gBitmapTexture.render(curX, curY, &mChars[ascii]);
...
bool loadMedia()
{
	...
	//Load font texture
	if( !gBitmapTexture.loadFromFile( "./resources/lazyfont.png" ) )
	{
