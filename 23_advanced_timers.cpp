...
// 手动控制帧率，但屏幕刷新率一般为60 Hz，所以建议使用同步技术，避免跳帧
...
void LTimer::pause()
{
    ...
    if( mStarted && !mPaused )
    {
        ...
        mPausedTicks = SDL_GetTicks() - mStartTicks; // 从开始到按下Pause的时间间隔
		mStartTicks = 0;
    }
}
...
void LTimer::unpause()
{
    ...
    if( mStarted && mPaused )
    {
        ...
        mStartTicks = SDL_GetTicks() - mPausedTicks; // 按下Pause后，直到Upause前的时间间隔
...
Uint32 LTimer::getTicks()
{
    ...
    if( mStarted )
    {
        ...
        else
        {
            ...
            time = SDL_GetTicks() - mStartTicks; // 减去无效时间（按下Pause到再次按下Unpause之间，属于无效时间）
        }
    }
...
bool init()
{
	...
	else
	{
		...
		{
			// 如果开启了 SDL_RENDERER_PRESENTVSYNC ，则最大帧率与屏幕刷新率一致
	...
	else
	{
		...
		{	
			...
			while( !quit )
			{
				...
				// 如果不支持硬件刷新率同步，可使用如下方案：
				/*
				int frameTicks = capTimer.getTicks();
				if (frameTicks < SCREEN_TICKS_PER_FRAME)
				{
					// 防止程序运行过快 Wait remaining time
